...
const UsersList = (props) => {
  ...
  const users = useMemo(() => {
    return props.users.map((user) => {
      ...
      if (last) {
        const {fromUser, read, text, data, time} = last
        m.name = fromUser._id === props.userId ? 'Вы' : fromUser.nickname || fromUser.login
        m.text = `: ${text}`
        if (dialog.wid !== props.userId) m.read = read
        m.unread = (() => {
          return _.countBy(dialog.messages, {
            read: false,
            fromUser: {_id: dialog.wid === props.userId || dialog.wid},
          })
        })()?.true
        m.time = time
        m.data = data
  ...
  return (
    <div className={classes.root}>
      <Toolbar bgcolor='primary'>
        {props.isOpen && (
          <IconButton className={classes.close} color='inherit' onClick={props.mobileClose}>
